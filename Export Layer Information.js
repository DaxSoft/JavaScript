...
'use strict';
    ...
    function addJson(hash) {
        ...
        this.string += hash.space + '"' + hash.key + '": ' + hash.value + "," + hash.break;
        return this.string;
    }
    // default rules
    ...
    $.data = {
        ...
        y:                  new Array(),
        blendMode:          new Array(),
        isBackgroundLayer:  new Array(),
        kind:               new Array(),
        positionLocked:     new Array(),
        filename:           new Array(),
        opacity:            new Array() 
    }
    ...
    function bringOutEachLayerVisible() {
        ...
        for (var i = 0; i < visibleLayers.length; i++) {
            ...
            $.data.isBackgroundLayer.push(current.isBackgroundLayer);
            $.data.positionLocked.push(current.positionLocked);
            if (current.name == ":") {
    ...
    function exportInformation() {
        ...
        for (var index = 0; index < $.data.name.length; index++) {
            ...
            string += addJson({key: "kind", value: '"' + $.data.kind[index] + '"'});
            string += addJson({key: "isBackgroundLayer", value: $.data.isBackgroundLayer[index]});
            string += addJson({key: "positionLocked", value: $.data.positionLocked[index]});
